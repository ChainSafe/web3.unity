...
public class Web3AuthWalletGUITokenManager : MonoBehaviour
{
    ...
    private async void SetTokens()
    {
        if (File.Exists(Path.Combine(Application.persistentDataPath, "customToken.txt")))
        {
            ...
            var balance = await ChainSafeManager.Web3.Erc20.GetBalanceOf(customTokenContract, ChainSafeManager.Web3.Signer.PublicAddress);
        ...
        // Set native token
        nativeTokenSymbolText.text = ChainSafeManager.Web3.ChainConfig.Symbol.ToUpper();
        var hexBalance = await ChainSafeManager.Web3.RpcProvider.GetBalance(ChainSafeManager.Web3.Signer.PublicAddress);
    ...
    private async void GetSymbol()
    {
        ...
        try
        {
            symbolTask = ChainSafeManager.Web3.Erc20.GetSymbol(customTokenAddressInput.text);
    ...
    private async void TransferTokens()
    {
        ...
        switch (selectedTokenToTransfer.value)
        {
            case 0:
                ...
                var response0 = await Evm.SendTransaction(ChainSafeManager.Web3, transferTokensWalletInput.text, amountInWei0);
            ...
            case 1:
                ...
                transferTokensContainer.SetActive(false);
                var response1 = await ChainSafeManager.Web3.Erc20.Transfer(customTokenContract, transferTokensWalletInput.text, amountInWei1);
