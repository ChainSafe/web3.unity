...
{
    public class StubReownProvider : WalletProvider
    {
        private readonly StubReownConfig config;
        private readonly IChainConfig chainConfig;
        private readonly IHttpClient httpClient;

        public StubReownProvider(StubReownConfig config, Web3Environment environment, IChainConfig chainConfig) : base(environment, chainConfig)
        {
            this.config = config;
        ...
        public override async Task<T> Request<T>(string method, params object[] parameters)
        {
            switch (method)
            {
                ...
                default:
                    // Direct RPC request via Reown RPC url.
