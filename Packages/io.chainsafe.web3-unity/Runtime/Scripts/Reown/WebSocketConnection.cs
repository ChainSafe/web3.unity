...
{
    /// <summary>
    /// Custom web socket implementation for Reown, based on NativeWebSocket https://github.com/endel/NativeWebSocket.
    /// </summary>
    public class WebSocketConnection : IJsonRpcConnection, IModule
    {
        ...
        async Task IJsonRpcConnection.Close()
        {
            ...
            ReownLogger.Log("Closing websocket due to Close() being called");
        ...
        async Task IJsonRpcConnection.SendRequest<T>(IJsonRpcRequest<T> requestPayload, object context)
        {
            ...
            try
            {
                ...
                ReownLogger.Log($"[WebSocketConnection-{Context}] Sending request {json}");
        ...
        private void OnMessage(byte[] data)
        {
            ...
            ReownLogger.Log($"[WebSocketConnection-{Context}] Got payload: \n{json}");
