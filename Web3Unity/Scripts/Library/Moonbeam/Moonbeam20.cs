...
using Moonbeam20Definition;
using UnityEngine;
using UnityEngine.Networking;

public class Moonbeam20
{
    ...
    {
        string url =
            host + "/moonbeam20/name?network=" + _network + "&contract=" + _contract;
        UnityWebRequest webRequest =UnityWebRequest.Get(url);
        await webRequest.SendWebRequest();
        Name response =
            JsonUtility
    ...
    TotalSupply(string _network, string _contract)
    {
        string url =
            host +
            "/moonbeam20/totalSupply?network=" +
            _network +
            "&contract=" +
            _contract;
        UnityWebRequest webRequest =UnityWebRequest.Get(url);
