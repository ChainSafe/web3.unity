...
namespace Web3Unity.Scripts.Prefabs.Ethers
{
    public class EthersExample : MonoBehaviour
    {
        ...
        async void Start()
        {
            ...
            // var provider = new JsonRpcProvider(dispatcher, "https://eth-mainnet.alchemyapi.io/v2/-xZN4CbntuRCIKrOSZgfdxUpV9zG_KtM");
            // var provider = new JsonRpcProvider(dispatcher); // default url http://localhost:8545
            var provider = new JsonRpcProvider(dispatcher, chains[1].RPC[3]);
            // var provider = new Web3Provider(dispatcher, new WebGLProvider());

            _provider = provider;

            provider.On("block", (ulong blockNumber) =>
            {
